...
export function isDueTomorrow(status: TodoStatus, date: string | number | Date | null | undefined) {
...
export function isOverDue(status: TodoStatus, date: string | number | Date | null | undefined) {
    if (!date || status != TodoStatus.ACTIVE) { return false; }
    const inputDate = new Date(date);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
